...
Usage:
    ...
    [--profile <value>]
    [--debug <value>]
    [--network-load-balancer-name <value>]
...
def get_nlb_data(elb_data, region, load_balancer_name, prefix, nlb_name,
        tg_name, ssl_hc_path):
    """
    ...
    for listener in nlb_data['listeners']:
        ...
        if tg_name :
            ...
            target_group['Name'] = re.sub('-$', '', load_balancer_name[:18])

        target_group['Name'] = target_group['Name'] + "-nlb-tg-" + \
...
def main():
    ...
    parser.add_argument("--network-load-balancer-name",
    ...
    parser.add_argument("--target-group-name", help="Base name for target groups",
                        required=False, default=None)
    parser.add_argument("--profile", help="The credentials profile name to use",
                        required=False, default=None)
    ...
    nlb_name = args.network_load_balancer_name
    ...
    if passed_hardfailure_detector(elb_data):
        ...
        if softfailurecheck_result[0]:
            ...
            nlb_data = get_nlb_data(
                elb_data, region, load_balancer_name, prefix, nlb_name,
                tg_name, ssl_hc_path)
            if args.dry_run:
