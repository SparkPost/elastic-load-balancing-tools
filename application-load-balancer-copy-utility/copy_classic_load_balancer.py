...
# [--profile <value>]
# [--debug <value>]
# [--application-load-balancer-name <value>]
...
def get_alb_data(elb_data, region, load_balancer_name, alb_name, tg_name):
    import re
    if debug:
    ...
    for listener in alb_data['listeners']:
        for target_group_attribute in alb_data['target_group_attributes']:
            ...
            if listener['TargetGroup_Port'] == target_group_attribute['TargetGroup_Port']:
                ...
                if tg_name :
                    target_group['Name'] = tg_name
                else :
                    target_group['Name'] = re.sub('-$', '', load_balancer_name[:23])

                target_group['Name'] = target_group['Name'] + "-tg-" + \
                        str(listener['TargetGroup_Port'])
                if 'stickiness.type' in target_group_attribute:
                    target_group['Name'] = target_group['Name'] + "-" + (target_group_attribute['stickiness_policy'])
...
def main():
    ...
    parser.add_argument("--application-load-balancer-name",
                        help="Name for network load balancer "
                             "instead of autogenerated",
                        required=False, default=None)
    parser.add_argument("--target-group-name", help="Base name for target groups",
                        required=False, default=None)
    parser.add_argument(
...
with Application Load Balancers, but do not perform create operations",
    ...
    alb_name = args.application_load_balancer_name
    ...
    if passed_hardfailure_detector(elb_data):
        if passed_softfailure_detector(elb_data):
            ...
            alb_data = get_alb_data(elb_data, region,
                load_balancer_name, alb_name, tg_name)
